...
status = discord.send(url:"%s/path/a/b/c", headers: {x:"a",y:"b",z:"c"}, data: bytes(v: "body"))
...
func TestDiscord_ValidationFail(t *testing.T) {
	script := `
import "discord"
discord.send(url:"http://127.1.1.1/path/a/b/c", headers: {x:"a",y:"b",z:"c"}, data: bytes(v: "body"))
`
